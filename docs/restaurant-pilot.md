...
- After checkout, the guest-facing tracking screen offers **Salir** with three explicit choices: save the consumption receipt on the device, open the device email client with the receipt, or leave without saving. Leaving removes the private account reference from that device.
- Restaurant rewards use points earned at that restaurant; AssetTrack-sponsored rewards use global points. A restaurant reward may be a menu product, a preset percentage discount with an optional cap, or a manually named custom reward.
- A waiter can transfer an open account to another dining table, takeout position, or bar seat without changing the guest link or merging independent accounts. Bar seats automatically remove table service; moving back to a dining table applies the destination's service policy. Every transfer is audited.
- Kitchen, bar, and waiter dashboards can enable and test operational alerts. New actionable work triggers sound, a vivid visual state, and vibration where the browser and device support it. Alerts are opt-in because browsers require a user gesture before playing sound.
- Every open account now carries its own responsible staff member. A bar transfer always selects the available bartender with the fewest active accounts. Without an available bartender, a waiter cannot complete the transfer; an administrator may explicitly assume it. The responsible bartender receives prepared items, confirms delivery, and closes the account, so moving the physical position never leaves the account ownerless.
- Ready products require a separate **Recibido de cocina o bar** confirmation before customer delivery. For kitchen products, this handoff freezes the kitchen-only elapsed-time counter; the final customer delivery remains a separate event.
- Waiter alerts repeat until the handoff is acknowledged. Kitchen and bar sound/vibration stop after three attempts while the visual alert remains until the item is accepted. Kitchen cards show a live green/amber/red elapsed counter based on the order threshold.
- The guest tracking screen offers optional calm sound and vibration alerts. Guests can independently disable sound or vibration; the visual treatment uses cool pastel colors and never imitates the red operational alarms.
...
1. Apply all migrations through `20260926070000_restaurant_account_responsibility` to a staging database, test the workflow, and confirm backup and restore procedures. The migrations preserve existing restaurant and scan records and add account ownership, preparation handoff timing, account transfers, bar positions, configurable reward types, alert support, catalog, promotion, fulfillment, invoice-request, and internal timing fields.
